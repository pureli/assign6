class Enemy {
  ...
  boolean isCollideWithFighter(int fighterX,int fighterY,int fighterW,int fighterH)
  {
    if(this.x + enemyImg.width >= fighterX && fighterX + fighterW >= this.x && this.y + enemyImg.height >= fighterY && fighterY + fighterH >= this.y)
    {
      return true;
    }
      return false;
  ...
  boolean isOutOfBorder(int screenW)
  {
    if(x>screenW)
    {
      return true;    
